CREATE OR REPLACE VIEW NumeroIngredientiMenu AS (
SELECT COUNT(distinct PS.IdIngrediente)AS NumeroIngredienti ,CM.IdMenu 
    FROM (ComponeMenu CM NATURAL JOIN Piatto P) INNER JOIN procedimentostep PS USING (IdPiatto)
    );

DROP TRIGGER IF EXISTS ControllaIngredientiMenu ;
DELIMITER $$

CREATE TRIGGER ControllaIngredientiMenu
BEFORE INSERT ON OffertaMenu FOR EACH ROW
begin

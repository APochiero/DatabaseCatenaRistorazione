...
DROP PROCEDURE IF EXISTS CercaConfezioniDaUsare;

DELIMITER $$

CREATE PROCEDURE CercaConfezioniDaUsare ( IN IDSede_ VARCHAR(10), IN DoseUsata_ INT, IN Ordine_ VARCHAR(6), IN Ingrediente_ INT )
BEGIN
...
FROM ( 
	...
	FROM
		Confezione C
			INNER JOIN Scaffale USING (IdScaffale)
            INNER JOIN
		Magazzino USING (IdMagazzino)
			INNER JOIN
		Sede USING (IDSede)
	WHERE
		IDSede = IDSede_ AND  ( STATO = 'Parziale' OR Stato = "Completa" )
